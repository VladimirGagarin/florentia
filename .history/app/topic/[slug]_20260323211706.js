...
export default function TopicScreen() {
  ...
  const renderSubtopic = ({ item, index }) => (
    <TouchableOpacity
      ...
      onPress={() => router.push(`/topic/${slug}/${item.toLowerCase()}`)}
      activeOpacity={0.7}
    >
      ...
      <Text style={[styles.subtopicName, { color: darkTheme.colors.text }]} numberOfLines={2}>
        {/*replace punctuations and capitalise every word if more than one  */}
        {item.replace()}
      </Text>
      <View style={[styles.exploreContainer, { borderTopColor: darkTheme.colors.border }]}>
  ...
  if (!topic) {
    return (
      <View style={[styles.container, { backgroundColor: darkTheme.colors.background }]}>
        <Text style={{ color: darkTheme.colors.text }}>Topic "{slug}" not found</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[
      ...
      <View style={[styles.descriptionCard, { backgroundColor: darkTheme.colors.surface }]}>
        <Text style={[styles.description, { color: darkTheme.colors.secondaryText }]}>
          {topic.description}
        </Text>
      </View>
